...
for i, b_diam in enumerate(branch_diams):
    for inh in ["tubes", "baloon"]:
        for k, s in enumerate(stim_types):
            ...
            ca_err = ca.std(axis=0)/ca.shape[1]**0.5
            ...
            if inh == "tubes":
                ...
                ax[k].fill_between(time[max_idx:len(time)//2:50]/1000,
                                   output[max_idx:len(time)//2:50]
                                   -ca_err[max_idx:len(time)//2:50],
                                   output[max_idx:len(time)//2:50]
                                   -ca_err[max_idx:len(time)//2:50], alpha=0.2)
            else:
                ax[k].plot(time[max_idx:len(time)//2:50]/1000,
                           output[max_idx:len(time)//2:50], color=colors_aging[i],
                           label=labels[inh]+" diam "+ str(b_diam))
                ax[k].fill_between(time[max_idx:len(time)//2:50]/1000,
                                   output[max_idx:len(time)//2:50]
                                   -ca_err[max_idx:len(time)//2:50],
